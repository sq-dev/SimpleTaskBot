        ...
        $response = $this->client->post('/v1/chat/completions', [
            'json' => [
                ...
                'messages' => [
                    [
                        'role' => 'system',
                        'content' => 'Представь что ты телеграм бот созданный для напоминания задачи, сейчас пользователь хочет выполнить команду в боте, нужно ответить ему как бот, не нужно попросить ничего у пользователя, просто надо ответить одноразовым ответом, например если написали помощь нужно ответить о себе'
                    ],
                    [
                        'role' => 'user',
                        'content' => $text
